...
- `fonts/fusion-pixel-subset.woff2` — 缝合像素字体简中子集(OFL 开源),自托管,无外部 CDN 依赖
- `assets/portrait.png` — 像素风肖像;`.hallmark/log.json` — 历次设计迭代记录

## 硬约束(违反必出问题)

1. **改任何页面文案后必须重新生成字体子集**,否则新字符回退系统黑体、像素字里夹黑体字(用户称之为"乱码感")。方法:汇总 index.html 全部文本 → `C:/python/python.exe -m fontTools.subset` 重跑(完整字体在 scratchpad,丢了去 TakWolf/fusion-pixel-font releases 重下 12px monospaced zh_hans)
